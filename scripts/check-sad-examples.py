...
A prefix-deriving SAD passes the single-hash test trivially — that IS step 2 of its own
algorithm — so the prefix is checked separately, never as a fallback.

A block whose `said` matches an already-verified block's is a SLICE of it (the wrapper example
shows four fields lifted from a whole SAD), not a whole SAD, and is reported as such.
...
Usage:
    scripts/check-sad-examples.py            # the SAD shape catalogue
    scripts/check-sad-examples.py PATH ...   # other Markdown files
...
DEFAULT_PATH = "docs/design/primitives/data/sad/shapes.md"
PLACEHOLDER = "#" * 44
DIGEST_CODE = "V"
...
def check_file(path, verbose):
    ...
    for index, raw in enumerate(JSON_BLOCK.findall(text), 1):
        ...
        if not isinstance(obj, dict) or "said" not in obj:
            if verbose:
                print(f"  skip  {path} block {index}  {label}  — no said")
            continue
        ...
        mode = "said"
        if "prefix" in obj:
            if derive_prefix_and_said(obj) == (obj["prefix"], obj["said"]):
                prefix_deriving += 1
                mode = "said + prefix (two-hash)"
            else:
                mode = "said (prefix inherited / foreign reference)"
        verified[obj["said"]] = index
        if verbose:
            print(f"  ok    {path} block {index}  {label}  — {mode}")

    slices = 0
    for index, label, obj in deferred:
        if obj["said"] in verified:
            slices += 1
            if verbose:
                print(f"  ok    {path} block {index}  {label}  — slice of block {verified[obj['said']]}")
            continue
        ...
        errors.append((f"{path} block {index}", f"{label} — {why}"))
