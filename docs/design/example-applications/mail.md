...
flowchart LR
  ...
  sapp -->|"lookup receive keys · deposit sealed<br/>message + payload to inbox nodes"| inbox
...
- **Send.** The sender resolves the recipient's published receive keys and inbox-node hints from the
  receive-key directory, seals once per recipient device, and deposits the message **to the
  recipient's own inbox nodes** — the `sadstore`s the hints name — so the sealed bytes live only
  where the recipient reads, off the federation. Attachments ride as the digest-named payload blob,
  uploaded against the deposited message.
