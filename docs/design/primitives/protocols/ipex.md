...
[Credentials](../../features/credentials/) _(forthcoming)_ is one caller; a shared-document
capability grant could be another. Confidentiality is **not** IPEX: to hide a disclosure from the
transport, seal an IPEX message inside an [ESSR](essr.md) envelope — integrity and attribution from
IPEX, confidentiality from ESSR, stacked at the edge.
...
- **Transport and delivery.** Routing, store-and-forward, the serve-time gate — the
  [exchange / mail](../../features/exchange/exchange.md) _(forthcoming)_ feature. IPEX defines the
  messages; the feature moves them.
...
- [`../../features/credentials/`](../../features/credentials/) _(forthcoming)_ — the first caller:
  what a disclosed SAD means, and the revocation and expiry checks IPEX delegates.
