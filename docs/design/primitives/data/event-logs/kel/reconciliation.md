...
that retention, when it changes the sink's **live-tip** set, is what moves its effective SAID and
drives digest convergence (a `RecoverRequired` cell still ingests the competing branch as evidence —
a live tip — even though nothing lands as a new canonical tip).
...
- **Divergent → Divergent sink.** The effective SAID is a **real digest over the live tips**, so two
  sinks converge **once they hold the same live tips** — anti-entropy exchanges the competing branch
...
- **Divergent with a canonically-landed `Rec`** — rejected with error. This state cannot exist
  through normal merge paths: synchronous resolution means an accepted `Rec` immediately commits a
  losing branch's root as `fork`, leaving a clean canonical chain. A divergent chain whose **live
  canonical** tables hold a `Rec` indicates possible DB tampering, and the partitioner refuses to
  propagate that. (Distinct from a `Rec` **retained as a competing privileged branch** — one the
  content-only guard rejected, which the chain counts as the second privileged branch of a
  `disputed` fork: that `Rec` is evidence and **must** propagate, like any other retained privileged
  branch — dropping it would split the reading across nodes.)
...
wire-format identifier, exchanged during anti-entropy. It is a **real digest over the live tips a
