...
| **Active (sealed)**    | The chain is Active (linear); this row covers a submission whose parent sits **strictly below** `last_seal_advancing_event` — in the locked portion (a stale view of the tip, or an attempted fork below the seal). The seal-cap rejects it as `SiblingLocked`, retaining the valid fork as evidence. Extending the seal event itself is a normal append and stays Active — only a **below-seal** parent is locked.                                                                                                                                                                                                     |
