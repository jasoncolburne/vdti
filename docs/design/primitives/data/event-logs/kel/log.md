...
| ------------------------------- | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `last_seal_advancing_event`     | `Rot` / `Ror` / `Rec` / `Wit` / `Trm` | Seal-cap — a new event's parent must sit at-or-after this serial (below is the locked portion; the seal event itself is a legal parent). The **derived seal**: the most recent to land **cleanly** (not a competing sibling); the reading computes against it from the events held, never arrival order. |
| `last_recovery_revealing_event` | `Ror` / `Rec` / `Wit` / `Trm`         | Spent-key rule — tracks the currently-committed recovery-key preimage; once a recovery-revealing event lands, that key is public.                                                                                                                                                                        |
