...
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Accepted**        | Linear extension; new tip established; seal advances on seal-advancing kinds.                                                                 | Events chain cleanly from the current tip (or from inception on an Empty chain).                                                                                                                       |
| **Diverged**        | A content event forks at an earlier serial; **Active → Divergent** (frozen).                                                                  | A content `Ixn` whose `previous` targets a pre-tip serial that holds a competing event (only the first is written).                                                                                    |
| **Recovered**       | Divergence resolved — the repairing branch kept, a losing **root** committed as the `Rec`'s `fork`; **→ Active**, seal advances to the `Rec`. | A `Rec` whose parent shape routes through the discriminator.                                                                                                                                           |
| **SiblingLocked**   | Not admitted as a canonical extension; no state change (retention is **separate**, witnessing-gated — below).                                 | Parent sits in the locked portion behind the seal (its serial holds a locked sibling), or a privileged landing would create / join a divergence; on a Terminated chain, the **sibling-to-`Trm`** race. |
| **KelTerminated**   | No state change. Submission rejected.                                                                                                         | Chains _from_ a `Trm` (parent kind `Trm`) — no kind admits a `Trm` parent (kind-schema rule, [§Routing order](#routing-order) rule 1).                                                                 |
| **RecoverRequired** | The content event is retained as evidence; does not resolve the fork; reading stays `forked`, origination frozen.                             | A live fork + a content event that neither resolves (`Rec`) nor buries (a winning-branch seal-advancer) it.                                                                                            |
| **Rejected**        | No state change; submission rejected.                                                                                                         | Structural-validation failure — the kind is inapplicable to the chain state (inception on non-empty, or non-inception on Empty).                                                                       |
...
structural checks (it never replaces them); on a witnessed chain it is governed by witnessing — the
...
- A losing **content** sibling on a witnessed chain is **prevented**, not turned into retained fork
  ...
  does not form.
- On a **direct-mode / solo** chain (no witnesses) nothing gates acceptance, so keep-all-data
  retains every competing branch as evidence, bounded to ≥ 2 per position (the rest droppable — a
...
a canonical extension with `SiblingLocked`. This is the structural rule that enforces
current-state-only authority — see
[§Forks are seal-bounded](../../../../protocol-doctrine.md#forks-are-seal-bounded) and
[`recovery.md` §Repair-event bound](recovery.md#repair-event-bound). Whether a rejected fork is
separately retained as evidence is witnessing-gated — the keep-all-data paragraph above.
