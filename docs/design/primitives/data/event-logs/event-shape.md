...
- Both bounds are re-checked on the post-delta roster at **every** roster-delta event — a user `Evl`
  or a federation `Wit` — not only at inception.
...
SAD (`roster`, `witnesses`, `folds`), a **single event-SAID pointer** (`bound`), or a **direct
...
| `roster`    | IEL `Icp` / `Evl` (user); federation `Fcp` / `Wit`                                                                                                                                    | a SAID → the roster/threshold **delta** SAD `{ add: prefix[], cut: prefix[], …changed thresholds }` — membership + threshold _changes_ only, never a full snapshot (IEL doctrine)                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `witnesses` | KEL `Icp` / `Wit`; IEL `Icp` / `Wit`; federation `Fcp` / `Wit`                                                                                                                        | a SAID → the witness-config SAD `{ threshold, signers }` (mandatory iff federated)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `folds`     | seal-advancing kinds — opt (present only when content is folded), req on a repair (KEL `Rot`/`Ror`/`Rec`/`Wit`/`Dec`; IEL `Evl`/`Del`/`Kil`/`Rpr`/`Dec`/`Wit`; SEL `Fld`/`Rpr`/`Dec`) | a SAD committing the content run folded since the prior seal — `{ canonical, forks[] }` plus the run's **boundary SAIDs** (so a spine walk **catches a naive `previousSeal` forgery** without expanding — necessary, not sufficient). `canonical` is `Ixn`-only, back-checked on expansion; absent when no content was folded. `forks[]` is **non-empty only on a repair** (KEL `Rec` / IEL·SEL `Rpr`) — it commits the archival tails the repair resolves; a non-repair carrier carries it only when content was folded (then `canonical` alone), omitting it otherwise (a `Dec` included) |
...
| `federation`    | Digest256 | KEL, IEL      | KEL `Icp` / `Wit`; IEL `Icp` / `Wit`                                                                                                                            | The federation IEL **prefix** a chain (KEL) or identity (IEL) binds to — which federation; follows the federation's evolution. The **identity's** authoritative binding lives on its IEL `Icp`/`Wit`; each member KEL's binding is field-matched to it (kind-strict `Wit ↔ Wit`). A SEL inherits its owner IEL's binding.                                                                                                           |
| `federationPin` | Digest256 | KEL, IEL      | KEL `Icp` / `Wit` (req), **opt on every KEL body event** (`Ixn`/`Rot`/`Ror`/`Rec`/`Dec`); IEL `Icp` / `Wit` (req)                                               | A **SAID** pinning the as-of federation position. Present = a forward **re-pin** within the inherited federation; absent = inherit the prior pin — so a same-federation re-pin rides whatever KEL body event the chain authors next (`Wit` is reserved for a **rebind** — it changes the `federation` prefix or the `witnesses` config). The prefix/SAID split: `federation` is _which_ federation, `federationPin` is _as of when_. |
...
| `Wit` | 3    | dual   | Federation (re)bind; carries `federation` / `federationPin` (+ optional `witnesses`); must change `federation` or `witnesses`; anchors the IEL `Wit`. Seal-advancing.                                  |
...
| `Pin` | `t_use`                                             | 1                     | `Ixn`                           | The **floor re-pin** — re-pins the SEL to its owner IEL's current tip, carrying only the top-level `pin` (no `manifest` / `previousSeal` / `folds`). Its job is the **serial-1 issuance floor** (the `Icp` can't hold a pin). Tier-1, repairable; **not** seal-advancing — it promotes nothing.                                                                                                                                                                 |
| `Fld` | `t_govern`                                          | 2                     | `Evl`                           | The SEL **re-seal** (a _pure fold_ — the KEL `Rot` / IEL empty-`Evl` analog, for a primitive with no roster or keys to evolve). Carries `previousSeal` + `folds`, caps the content run so a divergence repair stays page-atomic, and promotes below-fold content to durable. **Privileged, seal-advancing.** Anchored by an owner IEL `Evl` (which commits the `Fld`'s SAID via its `anchors` role) — a SEL fold lands at one of the IEL's own fold boundaries. |
...
`Dec`) cap its **local divergence/repair window** and carry `previousSeal` + `folds` like any spine.
Credential issuance, revocation, and status are a **feature** layered on the SEL primitive —
[`features/credentials/`](../../../features/credentials/) _(landed separately)_.
...
| `Rot` | req       | req          | fbd         | fbd          | fbd        | opt           | req          | opt (`anchors`, `folds`)                   |
| `Ror` | req       | req          | req         | req          | fbd        | opt           | req          | opt (`anchors`, `folds`)                   |
| `Rec` | req       | req          | req         | req          | fbd        | opt           | req          | req (`folds` w/ `forks[]`)                 |
| `Wit` | req       | req          | req         | req          | req        | req           | req          | req (`anchors`; `witnesses` / `folds` opt) |
| `Dec` | req       | fbd          | req         | fbd          | fbd        | opt           | req          | opt (`folds`)                              |
...
| ----- | ----- | ---- | ------------ | ------------------------------------------------------------------------ | ---------------------------- |
| `Icp` | req   | req  | fbd          | req (`roster`, `witnesses`; a federation `Fcp` adds `clock`)             | fbd                          |
| `Ixn` | fbd   | req  | fbd          | req (`anchors`)                                                          | fbd                          |
| `Evl` | fbd   | req  | req          | req (`roster`; `anchors` / `folds` opt)                                  | fbd                          |
| `Del` | fbd   | req  | req          | req (`delegates`, `folds` opt)                                           | fbd                          |
| `Kil` | fbd   | req  | req          | req (`anchors`, `folds` opt)                                             | req (`govern` \| `delegate`) |
| `Rpr` | fbd   | req  | req          | req (`folds` w/ `forks[]`; `anchors` opt)                                | fbd                          |
| `Dec` | fbd   | req  | req          | opt (`folds`; a federation `Dec` adds `clock`)                           | fbd                          |
| `Wit` | fbd   | req  | req          | req (`witnesses`; `folds` opt; a federation `Wit` adds `roster`+`clock`) | fbd                          |

The `nonce` (inception only) drives prefix unpredictability (§Prefix derivation). `pins` is the
...
threshold of members, so every IEL event carries it. The exact roster delta SAD and pins-SAD
schemas, the consent rule for additions, and the per-kind anchor matrix are IEL doctrine —
[`iel/`](iel/).
...
| `Fld` | fbd   | fbd   | fbd  | req | req          | req (`folds`)                          |
| `Dec` | fbd   | fbd   | fbd  | req | req          | opt (`folds`; `bound` on a rescission) |
| `Rpr` | fbd   | fbd   | fbd  | req | req          | req (`folds` w/ `forks[]`)             |
