...
|---|---|---|---|---|
| `governance` | `Digest256` | IEL, SEL | IEL `Fcp` / `Icp` / `Evl`; SEL `Icp` / `Evl` | SAID of a governance Policy SAD: the chain's **self-mutation** authority. On an IEL it gates the IEL's own lifecycle events (`Evl` / `Dec`, i.e. key, policy, and roster changes, and decommission); on a SEL it gates the SEL's lifecycle events (`Evl` / `Rpr` / `Dec`). Declared at inception; evolved via `Evl`. It is **never** what an external `iel(X)` leaf evaluates — that is `authentication`. |
...
**`Evl` validates the {roster, policies} pair.** An `Evl` may change any of {`governance`, `authentication`, `delegation`, `roster`, `delegating`} (≥ 1) and carries the full roster + policy state. The verifier enforces **referential integrity on the post-application pair**: every `mem(group)` in the three policies must resolve to a group in the post-event roster. So an `Evl` that drops a roster group still referenced by a policy is rejected — to retire a group, drop its policy references in the same `Evl` (or a prior one). Either may change in isolation; the only constraint is that the post-`Evl` pair still verifies. No committed state can hold a dangling reference.
...
The pin SAD is a **labeled collection of positional pinnings** — one array per SEL policy: `{ said, governance: [ … ], operation: [ … ] }`. The top-level labels are the policy names in the clear (event logs are public — nothing leaks); the slots inside each array are **positional** — one slot per prefix occurrence in that policy, in walk order per [`../policy/pinning.md`](../policy/pinning.md) — with no per-leaf labels, so policy internals don't leak. The common single-identity SEL (`governance = iel(P)`, `operation = iel(P)`) is a one-slot-each pinning.

The pin is set at `Est` (v=1) and ratcheted forward — parent-monotonic — by the governance-gated `Evl` (see [`../../../protocol-doctrine.md` §Per-Event Parent-Monotonic Ratchet](../../../protocol-doctrine.md#per-event-parent-monotonic-ratchet-sel-specific)). `Est` **self-supplies its resolution context**: the parent `Icp` carries no pin, so the only pin in existence at `Est` is the one the `Est` itself declares. The only structural bound on that choice is the upper `IelDivergent` rule (a pinned IEL event must sit at-or-below that IEL's seal); there is **no freshness floor** — a slot may reference arbitrarily old chain state (see the ex-member note in [`../../../protocol-doctrine.md` §Anchor Tier Elevation](../../../protocol-doctrine.md#anchor-tier-elevation)). The exact pin-SAD serialization and the multi-slot ratchet are [Open Items 1–2](#open-items).
