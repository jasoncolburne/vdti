...
- The bounds are re-checked on the post-delta config at **every** config-changing event — a user
  ...
  for `signers >= 2` the `signers − 1` leg is the one that binds, so a `signers` or `threshold`
  change is re-checked even when the roster is untouched (federation doctrine). A `Rpr`-cut (and any
  `threshold` it carries) is authorized at the **outgoing** `t_recover` (the pre-change gate — as a
  user `Evl` rides `t_govern`-of-outgoing — so a `Rpr` cannot lower its own gate before cutting).
...
| `roster`    | IEL `Icp` / `Evl` / `Rpr` (user); federation `Fcp` / `Wit`                     | a SAID → the roster/threshold **delta** SAD `{ add, cut: prefix[], …changed thresholds }` — membership + threshold _changes_ only, never a full snapshot. `add` is a **list** of prefixes on the user kinds and a **single prefix** on a federation `Wit` — one witness added per `Wit` (`Fcp` stands up the founding roster wholesale); `cut` stays a list everywhere (cuts remove synced witnesses — emergency multi-eviction is unaffected). One fresh witness alone cannot reach a majority `threshold` against synced co-selectees, so a roster transition never yields two unsynced witnesses co-signing (federation doctrine). On a user `Rpr` (the repair-and-evict fold) the delta is restricted to a **required non-empty `cut` + an optional `threshold`, never an `add`** — a cut-less, `threshold`-only, or `add`-bearing `Rpr` roster is malformed → rejected (IEL doctrine)                                                                                                                                                                                                                                                                                                                                                                              |
...
| `witnesses` | KEL `Icp` / `Wit`; IEL `Icp` / `Wit`; federation `Fcp` / `Wit`                 | a SAID → the witness-config SAD `{ threshold, signers }` (mandatory iff federated at inception; present-iff-changed on `Wit`). `threshold` sits above a structural **majority floor `threshold > signers/2`** — a strict majority of the selected witnesses, which makes two competing **content** siblings un-co-witnessable (a sub-majority config is rejected; `threshold = 1` is usable only at `signers = 1`, the lone-witness carve-out). A **user IEL's** content events must reach a majority quorum at their own `(IEL prefix, serial)` under this config, **alongside** their anchor-based authorization — the fork-prevention gate; the federation IEL is exempt (its every fork is privileged). Federation doctrine                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
repair `Rpr` additionally carries `fork`). Credential issuance, revocation, and status are a
...
| `Rpr` | fbd   | req  | req          | req (`fork`; `anchors`, `roster` opt)                                                | fbd                          |
