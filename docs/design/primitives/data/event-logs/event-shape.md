...
| `forks`     | a **repair only** — KEL `Rec`; IEL / SEL `Rpr` (req, non-empty)                | an inline **list** of the **losing-branch root SAIDs** the repair condemns — `[ said, … ]`, each entry a branch's **first divergent event** (a competing child of the fork point, off the retained chain); a named root condemns the branch's **entire subtree**, so growth after the repair is dead by descent. **Required and non-empty on a repair; forbidden on every other kind** (a non-divergent tip has nothing to repair). The retained (canonical) run is **not** committed — it is the linear chain `[previousSeal..previous]`, recovered by the flat walk (nodes keep full bodies; the flat query returns them), and "content was folded since the prior seal" is the derived predicate `previous != previousSeal`, no field. Validated, not trusted — the verifier checks each root off the full-span retained walkback (no self-condemnation), walks the condemned subtrees content-only, and independently walks the retained branches (+ the beacon), rejecting the repair if any such branch carries a privileged event |
