...
The exact on-chain roster-SAD schema and the composer-time flatten mechanics that expand `grp` into `id(member)` leaves are partly an implementation concern — settled in shape here, provisional in detail (as [`../policy/`](../policy/) flags).
...
**State vs. evidence — the pin is shallow by design.** The `policyPin` fixes *which version of each directly-named IEL* an event resolves against — the **membership / state layer**: shallow, full, floored, `id` / `grp` only, no nulls. *Who actually signed* — the expansion into a referenced IEL (its members, their devices) and the per-event anchor — is the **deep per-event authorizing evidence**, resolved by token-pass (§`policyPin` consumes verified chain tokens, not a live source — see [`../policy/evaluation.md`](../policy/evaluation.md)) into that IEL plus the authorizing event's own evidence pinning, **as-of authoring** (grandfather). That deep path is where the participating members' markers, `dev` prior-event SAIDs, and `null` slots live; its per-SEL-event shape is SEL-primitive (layer-4) detail, forward-pointed there, and the general evidence-pinning mechanism is [`../policy/pinning.md`](../policy/pinning.md). The pin names the snapshot; the evidence proves the signatures against it. **Deep does not mean unfloored:** each IEL state-marker the deep walk reaches is a **floored position on that entity's own chain** (composed by reference — every entity floors its own marker shallow, the depth is the tree of those floored chains), never an issuer's free choice. This holds **only because** every referenced entity is provisioned a registry-SEL **eagerly, at inception, regardless of credential activity** — a **layer-4 (IEL/SEL-primitive)** obligation; without it a never-issuing member (commonly a singleton device-holder reached by deep expansion) has no floor and the backdate re-opens one layer down. *Under that precondition* the only marker the rule leaves unfloored is the terminal **`dev`** anchor (per-event, self-dating — a leaked *current* key is the recovery residual, not a backdating surface). Registry-SEL **provisioning** is the layer-4 obligation; **issuance** as a floored `Ixn` on it is the **layer-5 credentials feature** — both forward-pointed.
...
| Cross-chain binding re-ratchet | (re-`Fed`) | — | `Evl` (re-ratchets `policyPin`; may also change `governance` / `operation`) |
