...
**event-log primitive reserves the three loci its own mechanisms define, and the substrate the
`topics/block` locus** (the `vdti/sel/v1/topics/*` rows — still no feature names).
...
| `vdti/sel/v1/topics/block`       | the substrate — a federation **prefix block** (`authority` = `id(federation)`, `data` = the blocked prefix); a reversible, witnessed-side per-prefix toggle SEL a federation uses to decline witnessing an abuser ([`../../../substrate/federation/blocking.md`](../../../substrate/federation/blocking.md))           |
