...
`target = hash('vdti/sel/v1/tags/rescission:{delegator}:{delegate}')` — a flat, domain-qualified
...
prefix derives from `(delegator, vdti/sel/v1/topics/delegation, delegate)` (`delegate` = `data`,
