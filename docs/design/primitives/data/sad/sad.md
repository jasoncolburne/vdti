...
What content the prefix commits to is per-primitive — KEL prefix commits to the whole inception SAD; the IEL prefix likewise commits to its whole inception SAD (the `authentication` / `governance` / `delegation` policy SAIDs, the `roster` on aggregate IELs, the delegator's prefix in `delegating` on delegated IELs, and the `nonce` whose randomness makes the IEL prefix unpredictable); the SEL prefix commits to `(governance, operation, topic)`. The shared rule is the fixed-value mechanism; the per-primitive shapes are documented in the corresponding event-log primitive docs.
...
The SAID is the load-bearing handle for every reference in the system — `previous` pointers, `policyPin` references (the pin itself is a SAD), policy SAIDs, anchor SAIDs, `manifest` / `document` SAIDs, custody references. The per-primitive event-log docs and [`../../../protocol-doctrine.md`](../../../protocol-doctrine.md) elaborate the structural rules that compose on top.
