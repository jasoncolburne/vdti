...
- **Forbidden on chain events is structural, not enforced.** Chain events have a fixed field set per kind; custody fields are not part of any chain-event kind's schema. A submission carrying inline `ownerIelEvent` or `readPolicy` on a chain event is rejected at the structural-validation pass of the merge layer ([`../../../protocol-doctrine.md` §Routing order](../../../protocol-doctrine.md#routing-order)).
