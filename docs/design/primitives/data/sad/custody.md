...
- **`readPolicy`** — a policy SAID that gates read access at fetch time. The referenced policy is fetched and evaluated against the verified prefix set from a signed read request. Composable via the policy DSL — `id(X)`, `thr(N, [...])`, and nested `pol` nodes are all permitted, so a single `readPolicy` can authorize arbitrary multi-identity read sets without those identities having to share an IEL. As a general policy, a `readPolicy` **forbids bare `dev`**: a device read routes through a singleton `id` that wraps the device. A bare `dev(K)` would keep granting to a device the owner has rotated out or decommissioned — acute in a read/custody context, where a retired device would still decrypt — whereas `id(X)` tracks the owner's current key state automatically. This forbiddance is **verifier-enforced at evaluation** (`evaluate_current_policy`, the read-side check): a `readPolicy` reaching a bare `dev` — written directly **or** behind a nested `pol(said)` → a `dev`-bearing SAD — is a **policy-validity error and the read denies**, fail-closed (a `dev`-bearing SAD is a singleton's own policy, which can't use `pol`, so the placement context never makes it legal — and an attacker-set `readPolicy` therefore cannot bypass the lifecycle defense; see [`leaf-semantics.md` §`dev`](../policy/leaf-semantics.md#devprefix--kel-key-match-tier-agnostic) and [`verifier-behavior.md`](../policy/verifier-behavior.md#verifier-behavior)). `None` for publicly readable content.
