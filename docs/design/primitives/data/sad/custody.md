...
**Custody is an application-level policy convention.** The write attestation resolves through `policies["custody"]` — an application-defined policy kind, not one of the three kinds the IEL primitive binds structurally (`"governance"` / `"delegation"` / `"sel"`). The IEL primitive doesn't enumerate `"custody"` because a SAD-store write is a storage-layer concern distinct from SEL interaction; applications register their own `policy_kind` strings under the IEL's open PoliciesSet extensibility model (see [`../event-logs/iel/`](../event-logs/iel/)).
...
- **Point-in-time (frozen).** Resolve the bound IEL event directly, read the `policies["custody"]` it tracks, and check the write attestation against that policy. The answer reflects the issuance-time governance of the writer's IEL — useful when the verifier wants to know "was this write authorized when it happened?"
- **Identity-current.** Dereference the bound event to its IEL prefix, walk the IEL chain to its current tip, and read the tip's `policies["custody"]`. The answer reflects the IEL's current state — useful when the verifier wants to know "is the writer's identity still authorized?"
