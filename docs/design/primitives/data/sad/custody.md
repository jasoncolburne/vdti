...
- **`readPolicy`** — a policy SAID that gates read access at fetch time. The referenced policy is fetched and evaluated against the verified prefix set from a signed read request. Composable via the policy DSL — `id(X)`, `thr(N, [...])`, and nested `pol` nodes are all permitted, so a single `readPolicy` can authorize arbitrary multi-identity read sets without those identities having to share an IEL. As a general policy, a `readPolicy` **forbids bare `dev`**: a device read routes through a singleton `id` that wraps the device. A bare `dev(K)` would keep granting to a device the owner has rotated out or decommissioned — acute in a read/custody context, where a retired device would still decrypt — whereas `id(X)` tracks the owner's current key state automatically. The permitted nested `pol` cannot reintroduce a bare `dev`: a `dev`-bearing policy SAD is a singleton's own policy, which can't use `pol` and so is never legitimately `pol`-referenced (see [`leaf-semantics.md`](../policy/leaf-semantics.md#devprefix--kel-key-match-tier-agnostic)). `None` for publicly readable content.
