...
Availability applies to **standalone SADs only.** Chain events (KEL, IEL, SEL) have a fixed kind-specific schema with no slot for `availability`; they replicate as indivisible units governed by their per-primitive event-log doctrine. Per-event availability is structurally impossible because the chain-event kind-schemas forbid the slot.
...
- **Availability** is an operational decision — where the bytes live, how long, whether retrieval is destructive.

Either axis composes independently with the other. The four-corners composition (custody-gated × replicated) is enumerated in [`custody.md` §Decoupling from availability](custody.md#decoupling-from-availability).
