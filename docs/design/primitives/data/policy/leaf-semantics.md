...
- the enclosing `[...]` is a **concat container**: multiple `grp` groups and single expressions mix freely — `thr(2, [grp(org, execs), grp(org, board), dev(K)])` flattens to one child list (`execs` members ++ `board` members ++ `dev(K)`).
