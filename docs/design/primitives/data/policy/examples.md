...
thr(2, [mem(directors)])
```
"Any 2 current directors authenticate as the aggregate." To require a specific individual, give them a one-element group — `mem(founder)` with `founder = {alice}`. A **singleton's** authentication, by contrast, is `kel`-only — e.g. `thr(2, [kel(device_a), kel(device_b)])` — the base case the `iel(...)` recursion bottoms out at (see *[IEL policy structure](iel-policy-structure.md#iel-policy-structure--aggregate-vs-singleton)*).
