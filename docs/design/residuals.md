...
| Granted remote's retired keys stay countable | Freshness · 6               | Signing             | Medium (180) | A key a **granted remote federation** retired, used before this federation's next horizon refresh (no local compromise)                                           | Post-refresh                      | Receipts from keys the remote no longer trusts still count here                                 | Refresh the grant — cadence is the dial                                                       |
| Rogue remote backdates past an un-grant      | Availability · 2            | Human Intent        | Medium (200) | A remote federation misusing its **own** keys after being un-granted — backdated τ inside the frozen horizon                                                      | **Yes** — reads stale             | Unwanted remote data keeps landing and being served                                             | Block the abusing prefixes                                                                    |
...
| Replicated-service member floods until cut            | Availability · 2            | Signing        | Low (60)        | Signed junk at link rate inside the compromise-to-cut window                                  | Per-peer rate cap; evict promptly — the flood is attributable by its own signature                                   |
| Restore loses the unreplicated window                 | Availability · 1            | Human Error    | Medium (300)    | A deposit no peer had pulled is gone; an absent peer can permanently miss the restored window | Take services down first; resubmission is the recovery; operator-forced rescan for the absent peer                   |
...
- **Mitigation** — The window is bounded by `MAXIMUM_WITNESS_KEY_WINDOW` (a key-window is closed at
  365 days regardless) and by the refresh cadence, which is the operator's dial: each refresh
...
- **Lost** — Up to the refresh interval (bounded by the auto-expiry) of countable receipts from keys
  the remote no longer trusts. Irreducible once any remote is granted; the dial is cadence, and a
  federation unwilling to carry it grants no remote trust.
...
- **Mitigation** — The cut is immediate against the case it exists for: an **honest** federation you
  ...
  refused at once. Against a rogue, four things bound it: the structural closure is the horizon —
  post-cut remote governance sits past `bound` and never accepts at all; fabrications read **stale**
  to every freshness consumer; their ingress rides the ordinary per-IP and request bounds; and no
  consumer's trust decision rests on them, since consumers keep their own configured set. Blocking
  the abusing prefixes is the local lever.
