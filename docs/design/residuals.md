...
**Severity** ranks how bad realization is, independent of how hard it is to reach — and it is
**bounded by the axis** the residual threatens (§Classification axes). Each axis carries a
**ceiling** — the worst a full break of that property can be — and a residual's severity sits at or
below that ceiling, by how _fully_ it breaks the property:

- **Trust — ceiling 16.** Integrity of authorship and attestation; a break is the core failure and
  is permanent (reincept). A full identity takeover is **16**; a buriable content forgery is ~**4**.
- **Privacy — ceiling 12.** Confidentiality of identities and relationships; a leak is
  **unrecoverable** — you cannot un-link a history — which is why it sits near trust. A
  whole-history correlation is ~**9–12**; a narrow "confirm a subject you already know" is ~**4**.
- **Freshness — ceiling 8.** Whether a consumer knows the current truth _in time_. A freshness miss
  is a wrong trust decision at the moment it matters — a trust consequence, but bounded, detectable,
  and transient, so roughly half of trust.
- **Recoverability — ceiling 6.** The cost of recovering from a realized compromise — bounded and
  known (reincept, lose history). It almost always co-occurs with a trust break (and then takes
  trust's ceiling by the max rule below), so its solo ceiling rarely governs.
- **Availability — ceiling 2.** Liveness; a denial or freeze is transient and recoverable, so it is
  the least of the five.

A residual that threatens **two** axes takes the **higher** ceiling, never the sum — a Trust +
Recoverability residual is bounded by trust's 16.
...
The **ranked summary** sizes each residual by **Severity** (how bad the outcome is, bounded by its
axis ceiling), **Exploitability** (how attainable it is), and **Risk = Severity × Exploitability**
(the band); it **sorts by Severity** and tags each by **axis** (the property at risk) and
...
- **Privacy (ceiling 12)** — confidentiality of identities, relationships, and activity. Can an
  observer correlate, de-anonymize, or read what should be hidden?
- **Availability (ceiling 2)** — liveness. Can the system, an identity, or an operation be denied,
  frozen, stalled, or capped?
- **Recoverability (ceiling 6)** — the cost of recovering from a realized compromise or error. Is
  ...
  revoke?
- **Freshness (ceiling 8)** — whether a consumer can know the current truth in time. Is an attack
  real but unseen at decision time — an eclipse, a stale view, a drifted clock, a still-open
  harvested-key window?
...
| Residual                              | Severity | Exploitability      | Risk         | Axis                   | Requirements                                                                                                                                                      | Detectable                     | Outcome                                            | Resolution                                                    |
| ------------------------------------- | -------- | ------------------- | ------------ | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ | -------------------------------------------------- | ------------------------------------------------------------- |
| Rotation-reserve theft                | 16       | Reserve             | Medium (160) | Trust + Recoverability | A `t_govern` or `t_authorize` reserve quorum                                                                                                                      | **Yes** — monitoring           | Attacker takes over your prefix                    | Reincept + notify relying parties                             |
| Signing-key theft + witness collusion | 16       | Signing + Witnesses | Low (32)     | Trust + Recoverability | `t_use` signing keys **and** a colluding `threshold` witness quorum, shrinking toward `2·threshold − signers` as a partition splits the redundancy onto the rival | **Yes** — provable double-sign | Your identity bricks                               | Reincept                                                      |
| Document governance-quorum compromise | 12       | Reserve             | Medium (120) | Trust + Recoverability | A `t_authorize` quorum of the creator's reserves                                                                                                                  | **Yes** — monitoring           | The document is captured                           | Reincept + notify relying parties                             |
| Eclipsed at decision time             | 8        | Signing + Witnesses | Low (16)     | Freshness              | A signing-key fork + coercing the queried witnesses to withhold                                                                                                   | Post-resolution                | You bind the attacker's branch, not the honest one | Re-verify multi-source before binding                         |
| Just-cut key still reads fresh        | 6        | Signing             | Medium (180) | Freshness              | Harvest a just-cut key within the staleness window (seconds)                                                                                                      | **Yes** — stale on close       | A just-revoked key still forges, briefly           | Window closes/tighten thresholds                              |
| Lookup prefix seen by witnesses       | 4        | Human Intent        | Medium (400) | Privacy                | Compromise a witness operator to weaponize a prefix it legitimately sees, + a known candidate subject                                                             | No — passive at the witness    | Infra confirms a subject you both know             | Inherent to a witnessed lookup; a cut stops only exfiltration |
| Signing-key content forgery           | 4        | Signing             | Medium (120) | Trust                  | `t_use` signing keys (content stays buriable)                                                                                                                     | **Yes** — fork + monitoring    | Forged content appears until you bury it           | Bury (rotate) — the anchors die on ascent                     |
| Forced-dead receive key               | 2        | Reserve             | Low (20)     | Availability           | A `t_authorize` reserve quorum — forge a `Trm` rescind so the key reads dead                                                                                      | **Yes** — monitoring           | Senders can't reach you until you republish        | Republish at a fresh lineage                                  |
...
| Residual                                              | Severity | Exploitability | Risk            | Axis                   | Outcome                                                            | Mitigation                                                                                                           |
| ----------------------------------------------------- | -------- | -------------- | --------------- | ---------------------- | ------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| Lost keys left in the roster                          | 16       | Human Error    | Critical (4800) | Trust                  | Federation governance is taken over                                | Cut lost keys promptly (watch the at-risk flag)                                                                      |
| Single-device IEL full compromise                     | 16       | Signing        | Medium (480)    | Trust + Recoverability | A full device compromise takes over your prefix                    | Run a ≥3-device IEL (survivors meet `t_govern` to cut a compromised device)                                          |
| Skipping the freshness check                          | 12       | Human Error    | Critical (3600) | Trust                  | You accept a forged, stale-issuer credential                       | Always run the mandatory to-tip check                                                                                |
| Gated record without a nonce                          | 12       | Human Error    | Critical (3600) | Privacy                | A named member is de-anonymized                                    | Give every gated record a high-entropy nonce                                                                         |
| Under-provisioned witness set                         | 12       | Witnesses      | Low (60)        | Trust                  | N compromised witnesses forks you                                  | Provision `signers`/`threshold` for the fork-cost `N = 2·threshold − signers` you need (traded against availability) |
| Leaked chain prefix                                   | 9        | Human Error    | Critical (2700) | Privacy                | Your whole history becomes linkable                                | Keep prefixes out of logs and shared refs                                                                            |
| Fail-open revocation opt-out                          | 8        | Human Error    | Critical (2400) | Trust                  | You accept a revoked subject                                       | Stay fail-secure; don't opt down                                                                                     |
| Fail-open on a walk timeout                           | 8        | Human Error    | Critical (2400) | Trust                  | You accept a revoked subject under latency                         | Fail secure on timeout where it matters                                                                              |
| Guessable derived address                             | 6        | Human Error    | High (1800)     | Privacy                | An attacker probes your status / existence                         | Use a high-entropy `data` input                                                                                      |
| Leaked gated-record bytes                             | 6        | Human Error    | High (1800)     | Privacy                | Gated plaintext is readable once bytes escape                      | Encrypt sensitive content (use the exchange channel)                                                                 |
| Consumer clock drifts backward                        | 6        | Human Error    | High (1800)     | Freshness              | You accept backdated data unknowingly                              | Keep the clock NTP-synced within `CLOCK_TOLERANCE_BAND`                                                              |
| Never-rotated witness key                             | 6        | Signing        | Medium (180)    | Freshness              | A stolen witness key forges up to a year                           | Rotate witness keys with margin                                                                                      |
| Mis-set rescission boundary                           | 5        | Human Error    | High (1500)     | Recoverability         | You cut honest work or miss bad work                               | Cut at genesis when the loss time is unknown                                                                         |
| Naive delegator rescission                            | 5        | Human Error    | High (1500)     | Recoverability         | Sub-delegated creds keep being issued                              | Move the boundary before the sub-grant                                                                               |
| Routing around a delegator                            | 5        | Human Error    | High (1500)     | Recoverability         | A cred via another path stays valid                                | Rescind at the root, or issue under a threshold                                                                      |
| Terminated identity freezes revocation and rescission | 3        | Human Error    | High (900)      | Recoverability         | A retired issuer/delegator can't revoke, rescind, or close periods | Revoke and rescind before terminating                                                                                |
| Recovery breaks a dependent                           | 3        | Human Error    | High (900)      | Recoverability         | A dependent event breaks                                           | Don't bury a branch your own anchors depend on (you shouldn't erase your own events)                                 |
| Two-member identity                                   | 2        | Human Error    | High (600)      | Availability           | One bad device freezes you; reincept                               | Add a third key to become recoverable                                                                                |
| Anonymous-write flood                                 | 1        | Human Error    | Medium (300)    | Availability           | Your store fills with junk (until gated)                           | Rate-limit or gate anonymous writes                                                                                  |
| Even-signers tie                                      | 1        | Human Error    | Medium (300)    | Availability           | A position stalls (never forks)                                    | Use an odd number of signers                                                                                         |
