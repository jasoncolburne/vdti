...
| Self-lane message backdate is detectable, not prevented | Recoverability | A current chat member can fork its own lane to inject a message dated within (its key window ∩ the epoch it held); `(epoch, timestamp)` monotonicity makes a tip-append malformed, so injection requires a self-signed fork — undeniable once both siblings reach a common member (an eclipse / split delivery only defers detection, never hides it), confined, never forward |
