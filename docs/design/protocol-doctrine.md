...
The pin ratchets forward, **parent-monotonic**: no slot's pinned chain position may regress relative to the parent's pinning; a pin-only `Evl` must still advance ≥1 slot (see [§Shape constraints on SEL Evl](#shape-constraints-on-sel-evl)). Slot correspondence across a policy change and subset-advance semantics are SEL-primitive doctrine (event-shape Open Item 1). Only `Est` / `Evl` carry a pin, and both land only as clean linear extensions (see [§Privileged Divergence is Terminal](#privileged-divergence-is-terminal)), so the comparison is always against the submitter's linear parent.
...
- **SEL Evl-Evl change rule.** A consecutive `Evl` must change ≥1 of {`governance`, `operation`, `policyPin`} (a no-op `Evl` is rejected). The pin is **parent-monotonic** — no slot's pinned event may regress (see [§Per-Event Parent-Monotonic Ratchet (SEL-specific)](#per-event-parent-monotonic-ratchet-sel-specific)); when the pin is the field changing, ≥1 slot advances. A governance/operation-only `Evl` re-stating `policyPin` equal is legal.
