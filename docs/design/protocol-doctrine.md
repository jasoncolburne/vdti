...
2a. **Hard auth at landing.** Governance / signature check hard-fails on rejection at the merge layer; no soft-fail.
   ...
   - **SEL Rpr**: governance evaluation against the bound IEL event's tracked `governance` hard-passes threshold. Repair-event SAID is anchored by a tier-3 KEL event per contributing governance member.
...
**Policy satisfaction under elevation.** The policy DSL has leaf nodes (`kel(prefix)`, `del(prefix)`) that test anchor presence and internal nodes (`wgt`, nested `pol`) that compose leaf results. Under anchor elevation, the leaf-level anchor check requires the hosting KEL event to be of capability at-least the tier specified for the dependent event; a leaf that finds an anchor of insufficient capability evaluates as unsatisfied. DSL composition is unchanged — `wgt` still sums satisfied-child weights against the minimum; `pol` still recursively resolves and evaluates; `del` still requires a delegate's anchor on the SAID. The verifier accepts the event when the top-level policy evaluates as satisfied, where satisfaction is computed against the tier-appropriate anchor check at every leaf.
...
- **Policy-layer separation (when `authentication ≠ governance`).** `Sea` is governance-authorized. An auth-key-only holder can produce `Upd`s but not `Sea`. Multi-device or otherwise composed identities get real cryptographic separation between "auth-set wrote this" and "governance-set sealed this."
...
The exposure-window property is the load-bearing one and applies even to degenerate single-KEL IELs where `authentication = governance`.
...
The federation itself is an instance of the primitive that depends on convergence. A VDTI federation is a single IEL — the *federation IEL* — whose `authentication` declares the set of member identities authorized to participate in the witness mesh. On every node, the federation IEL and the supporting member KELs are replicated to the local `vdtid` instance (which hosts both SAD storage and the chain-log acceptance rows); `witnessd` queries `vdtid` as its source of truth for federation state. Propagation uses the normal mechanics — announcement-driven primary path, dependency tracking for out-of-order arrivals, anti-entropy as fallback. The federation has no separate consensus algorithm and no central state machine.

Convergence is what makes this work:

- **Identical `authentication` view across nodes.** Every node deterministically resolves the federation IEL's tip from the events it holds. If two nodes hold the same event set, they compute the same effective SAID and read the same current `authentication`. Anti-entropy converges any two nodes whose effective SAIDs differ.
- **Handshake authorization is path-agnostic.** A connecting peer's identity is checked against the federation IEL's current `authentication`. Two nodes that both hold the federation IEL's authentic tip produce identical authorization decisions.
- **Membership evolution converges.** A governance-authorized `Evl` event evolving `authentication` propagates via the standard IEL channel. Two nodes that have both received the `Evl` see identical post-evolution policy.
...
**Adversary-extended linear chains.** If an adversary captures KEL signing-key material (or SEL `authentication` material) and extends the chain linearly (`v_N`, `v_{N+1}`, …, `v_M`), the legitimate party's local chain after gossip has `v_M` as its highest-serial event. The legitimate party has no protocol move that targets `v_M` without endorsing the adversary's events — any submission extending `v_M.said` would attest to it. The structurally available moves all extend `v_{N-1}` (the legitimate party's last attested event):
...
**Federation IEL self-signing carve-out.** The federation IEL itself is witnessed under self-signing semantics at v=0, dispatched by IEL kind. At `Fcp` (v=0), the witness pool is the `Fcp` event's own declared `authPolicy.identity_leaves` (the founder identity IEL prefixes). At v > 0 (`Evl` / `Dec`), the standard rule applies (pool = prior-state `authentication`). The kind discriminator makes the self-attesting case structurally explicit. Identity IELs (and other non-federation IELs) use `Icp` at v=0 and are witnessed under their parent KEL's federation — the kind discriminator tells the verifier which rule to apply.
