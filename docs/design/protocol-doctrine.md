...
4. **Kind-specific.** Per-kind authorization (signing, dual-signing, anchor-tier checks against IEL-resolved policy), structural-extension rules (Evl change-≥1 + `policyPin` per-entry non-regression, inception batch rule, etc.), and chain-state effects (seal advance, decommission, repair).
