...
- **Federation binding = the prefix/SAID split** [inv 7, 14]: **`federation`** = the federation IEL **prefix**
  ...
  via `Wit`). **Two inception roots, never sequenced:** `Fcp` (federation infrastructure — genesis or added) **or**
  `Icp` (a user device) — **never `Fcp → Icp`**. An `Fcp` **is bound by founding/joining**: its v=1 is
  a **`Rot` that anchors the federation IEL `Fcp`** (the federation inception marker — kind-strict, T2 ↔ T2) in the
  **dependency-ordered pair** (`Fcp` v=0 → `Rot` v=1; not atomic — revised 2026-07-14) — or, for an added witness, its
  **consent `Ixn` anchoring the admitting federation `Wit`** (cold-6 F4) — the witness is bound by being **named in the roster it
  founds / joins**, never self-bound (governed *into* the roster — 2026-06-28). Genesis pattern: founder `Fcp → Rot`, the
...
- **The identity bond (cold-6 F4, 2026-07-22 — ratified live):** every KEL serves **exactly one identity**, declared
  by its **serial-1 event anchoring that identity's establishment act** — genesis witness `Rot` → federation `Fcp`;
  added witness `Ixn` → the admitting federation `Wit`; initial roster member `Rot` → IEL `Icp`; added member
  `Ixn` → the admitting `Evl`. Permanent (a chain never re-bonds); enforced by two event-local checks: (i) the KEL
  verifier — serial 1 anchors exactly one identity-establishment act; (ii) roster-delta validation — an admitted
  prefix's serial-1 bond names **this** identity (its inception or this admitting act; a previously-admitted chain
  fails — its serial-1 names a *prior* act — so **re-add = a fresh chain**, replace-don't-resume). **Root kind =
  service class:** a federation roster admits only `Fcp`-rooted chains, a user roster only `Icp`-rooted — load-bearing
  (without it an `Icp`-rooted chain whose serial-1 anchors a federation `Wit` would pass the bond check while breaking
  root-keyed `Wit`-facet dispatch); with it root kind and bond target agree at the gating moment, and the `root_facet`
  machinery is untouched. Consequences: membership in two identities through one chain is **unconstructable** (no
  cross-identity correlation through a shared chain; between acts the only lawful binding mismatch is the migration
  lag); **identity reincept ⇒ fresh member chains** (old bonds name the dead prefix — a clean break, the compromise
  may have touched member devices); chains/identities are deliberately cheap — per-app identities, per-identity
  chains (the correlation-resistance doctrine). Prior art: receive-key-directory's "one member of one"
  (assumption → enforced). Design: `kel/events.md` §The identity bond.
