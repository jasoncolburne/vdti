...
| `Gnt` (grant) | `t_authorize` | **T2** | **`Ath`** | **yes** | **sealed on arrival** — **seals a typed value**: `manifest.grant` names a grant-value SAD kinded under **`vdti/sel/v1/grants/*`** (owner-first naming, any kind ≤ **64 chars**). Instances: the document-membership grant-doc `G` (`grants/document-membership` — opens editor/commenter periods, shared-documents §1); the **read**-membership grant (`grants/document-read-membership` — opens reader periods, structurally only `readers`); a **directory receive-key** (`grants/directory-ml-kem-1024` — the value-bearing lookup §1f, vdti-area-receive-key-directory). Non-buriable; walked back by a `Dth` rescission, never overturned. |
