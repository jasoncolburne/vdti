   ...
   **Role vocabulary:**
   - `anchors` — lower-layer SAD / event SAIDs this event commits to below. **Carried by** KEL `Ixn` (required, ≥ 1) /
     ...
     *declaration*, the kind-strict `kills` role. *(A bare issuance commitment is a **flat hash with no event behind
     ...
     R5/R1.)*
   - `roster` — the roster/threshold (delta) SAD (IEL `Icp`/`Evl` (user) + `Fcp`/`Wit` (federation) — the federation's `Wit` carries its roster delta + `clock` — its **`add` is a *single* prefix** (one witness per `Wit`, except `Fcp` inception; `cut` stays a list — cold-seam P5, 2026-07-02: ≤ 1 unsynced witness per transition can't reach a majority `threshold`, closing the multi-add straddle); E1, 2026-06-28. **Eviction rides a user `Evl` carrying a `cut`** (first-seen — there is no `Rpr`; an urgent eviction is an ordinary governance `Evl` with a required non-empty `cut`, an `add` rides a later `Evl`; priced the outgoing `t_govern`, the post-cut roster re-checked against the inv 12 bounds — the security floor, the recoverability ceiling, the majority floor, the roster cap 32, and the roster-floor `|roster| ≥ 1`). *(`pins`/`pin` are **not** manifest roles —
   ...
   - *(The **`fork` role is DELETED (first-seen, 2026-07-08)** — there is no repair event, so nothing commits a
     ...
     witnessing rule — inv 13/17), and a **sealed** loser is never buried (≥ 2 sealed → `disputed`, can't bury a
     rotation). The **retained (canonical) run is NOT committed** — it is the linear chain `[previousSeal..previous]`,
   ...
   *(There is no SEL `Fld`/`Rpr` — no re-seal, no repair.)* **Tier-elevation (anchor tier ≥ event tier) is an *additional* floor, not the check** —
   ...
   - **KEL→IEL (member participation):** each IEL event is anchored **only** by the member-KEL kind that reveals
     ...
     **federation-governance** facet matches **only the witness-config** (the "witnessing stuff") — the **roster
     ...
     constrained monotonic + `≤ now+band` — inv 4 `clock` / federation §1f). The IEL `Trm` and the federation
...
12. **IEL authorization is a threshold vector** `{t_use, t_govern, t_authorize}` (the **count** axis,
    ...
    evicts). **Bounds (F-K, hardened 2026-06-21; majority floor + roster cap added 2026-07-08):**
    - **`t_use ≥ 1`** — `t_use = 1` is **single-device by choice** (no content resilience); `t_use ≥ 2` makes a
      single compromised device unable to author content (F10a). **`t_use` is exempt from the majority floor**
      (content is first-seen / recoverable).
    - **The roster is hard-capped at 32** (a DoS backstop — the verifier rebuilds the roster in memory as it walks;
      any delta pushing the live set past 32 is rejected; **all IELs, federation included** — federation §1a).
    - **The majority floor: `t_govern`, `t_authorize > |roster|/2`** (2026-07-08 — a strict majority signs every
      governance / grant, so any two authorizing quorums overlap (pigeonhole) and a governance fork always names a
    ...
    **VALID**: `≥ 2` forces every authority threshold to 2 (secure — no single-device seizure), and it simply can't
    ...
    bound is rejected. **A third, absolute floor — the roster is never emptied: post-delta `|roster| = |roster| + |add| − |cut| ≥ 1`** (beneath the security floor / singleton exception; the roster is a **set**, so `add ∉` the current roster, `cut ⊆` it, `cut ∩ add = ∅` — the size arithmetic holds; the same set discipline the federation states at §1a (cold-9 Q2 / cold-14 F1), general to *every* roster delta). A `cut` that would zero the roster is rejected — **one general rule** that makes *every* singleton IEL's roster **downward-immutable**: a singleton `cut` `Evl` (no `add`) computes `1 + 0 − 1 = 0` (`< 1`) → rejected, so a singleton can't be zeroed — **not only** the federation-member key-SEL owner (federation §1e). It still **allows** singleton evict-and-replace via an `Evl` — `cut 1 + add 1` stays `1`, **authorized at `t_govern`-of-outgoing** (the *old* member signs, so this works only for a planned migration where the sole key is **uncompromised/available**; a lost or compromised singleton can't produce that signature → reincept); only the zeroing is forbidden. **For the federation this re-check also covers the witness-config recoverability cap** — the **full** post-delta `threshold ≤ min(|roster| − 2, signers − 1)` **plus the majority floor `threshold > signers/2`**, re-applied on **any `Wit` that changes roster, `threshold`, or `signers`** (not a roster `cut` alone) — so a **bare `cut`** that would strand the federation un-recoverable (`|roster| 5→4` at `threshold 3`), a `t_govern`-hostage (`|roster| 4→3` at `t_govern 3`), **or a `signers`/`threshold` change landing on the *binding* `signers − 1` leg** (`{s 4, t 3}@5 → {s 3, t 3}@5` passes `|roster| ≥ threshold + 2` yet violates `t ≤ min(3,2)`, cold-seam F1) is **rejected**, forcing evict-and-replace or a simultaneous threshold-and-`signers` drop. **This is what actually enforces the `[locked]` "the witness IEL can never be brought to an unrecoverable size" above** (cold-13 F1; the roster-leg-only phrasing corrected to the full `min` — cold-seam F1, 2026-07-02 — since for `signers ≥ 2` the `signers − 1` leg binds and the roster leg is slack).
...
13. **Divergence is permanent and visible; it is scoped to T1 content; recovery is a plain rotation (first-seen,
    ...
    **Recovery — the universal rule (first-seen):** you attach a **burying seal-advancer at your last good event** —
    a `Rot` on the KEL, a governance seal (an `Evl`, or a `cut` `Evl` when it also evicts) on the IEL — retaining your
    ...
    **A kill-anchor (`Rev`/`Dth`) is NOT terminal** — it seals a kill on a *target*, not its host IEL, so the IEL
    ...
    survives, the content buried + re-issued), terminal only when it is one of ≥ 2 sealed branches. To both resolve a
    ...
    **is any competing branch you must bury a *sealed* branch (a `Rot`, `Evl`, `Ath`, or `Rev`/`Dth`)?** **No** (all
    ...
    out-seal it — a second sealed branch is `disputed`, terminal) → **reincept**. So the rotation reserve defends the
    ...
    **The disputed / data-local view (node-agnostic):** the recovery check above is *party-relative*; the symmetric,
    node-agnostic condition is **branch-level** — **≥ 2 branches each carrying a *sealed* event past the fork** is
    **disputed**, terminal for *everyone* (any party retains only its own branch, so a second sealed branch always
    ...
    federation **propagates**, never decides). A **sealed** event in **any** competing branch forces `disputed` (every
    sealed event is a seal-advancer → a spine fork — walk-independent), so an omitted `Rot` can't be hidden by sealing
    past it; unnamed content branches close below the seal + by descent (inv 4). The seal-advancing events form a
    ...
    **competing sealed branch it did not author** (a key-state branch you didn't author is un-buriable → the point of
    ...
    `disputed` (≥ 2 sealed branches, node-agnostic) forces reincept on *everyone*, while a *single* sealed branch you
    ...
    **Trust on a divergent chain — the seal is the boundary (walk/trust, locked 2026-06-22).** What a consumer may
    ...
    sealed *linear* events are immutable); **a competing *key-change* (sealed) branch that forks below the seal is
    a spine fork and still forces `disputed`** — it is never inert (burying it would resurrect a rotation; inv 13 F4).
    ...
    **wrong** — corrected 2026-07-05, freshread-8 E; a revocation moreover rides the issuer IEL (its `kills[]`
    ...
    (anchor-monotonicity), so two parties revoking one cred only forks if the issuer IEL forked — resolved by the IEL burying seal.)* It makes F-F's scenario *unreachable*: a governance event can never sit
    *above* an **unresolved** divergence, because a node originates nothing onto a live fork — the only append onto
    one is a **content-fork-burying seal-advancer** (which resolves it), never new governance. (A divergence with
    **≥ 2 sealed branches** — e.g. `{Evl, Evl}` — is disputed → reincept; a `{Evl, content}` collision is
    ...
    **Consequence — a network split + governance (F7, refined 2026-06-21):** if the network splits, one half can
    ...
    — **not** an automatic brick. Only **`{Evl, Evl}`** (both halves do governance → ≥ 2 sealed branches) is
    **disputed → reincept**.
...
17. **The spine overlay + keep-all-data make divergence detectable from the data (2026-06-23).** The seal-advancing
    ...
    - **Two views, one dataset.** The spine is verified by the **same walk algorithm** with `previousSeal` substituted
      for `previous`, yielding authority + a **terminal-divergence** view (a spine fork = two competing seals = sealed = terminal) but **not recoverable content forks or content completeness** (that needs the flat
    ...
    - **Detection lives on the flat chain (the guarantee); the spine is a convenience pre-check.** Terminal = **≥ 2
      branches each carrying a sealed event past the fork** (inv 13) — a **data-local** walk over **retained**
    ...
    - **Burial by position + descent — growth-proof (first-seen, 2026-07-08).** There is no repair event and no
      ...
      losing branch a lagging node grows after the burial is dead by descent, no follow-up). Each dead **lineage** is
      **depth-capped at 64 events past the last seal** (a deeper event forces a seal-advancer, sealed → `disputed`);
      ...
      content sibling at a position and declines later ones; **sealed** siblings witnessed up to **two** per position
      — two prove `disputed`, then declined — federation §1e). Dead events are **propagated + retained** but never
      canonical. A **sealed** branch below a seal is **never buried** (≥ 2 sealed → `disputed` — can't bury a
      rotation); a self-burial (a burying seal-advancer siblinging the retained chain, known from `previous`) is
    ...
    - **The beacon propagates; the data decides (inv 14).** Witness receipts **enumerate** the competing branches so
      ...
      re-validates** ≥ 2 sealed branches forces `disputed` immediately (threshold-independent), while a node
      holding only a **receipt** waits for the **witness threshold** (sealed; a losing *content* sibling never
