...
- Canonical: only `docs/design/primitives/data/event-logs/event-shape.md` (pre-reshape); **no `iel/` dir exists yet.**
...
- **Threshold declaration (locked 2026-06-25).** The **`Icp` declares the active threshold set** — exactly the
  ...
  (`Ixn`→`t_use`, `Ath`→`t_authorize`, `Rpr`→`t_recover`, `Evl`/`Wit`/`Trm`→`t_govern`). A **user** IEL → `t_govern` +
...
- **Read the manifest kind-first (inv 4 F1) — load-bearing.** Each kind may carry **only** the roles in
  ...
  directions (a `Rev`/`Dth` anchors only `Trm`s, an `Ixn` only content/v1, an `Evl` only `Fld`s, an `Rpr` only `Rpr`s —
...
- **`event-shape.md`** IEL sections are pre-reshape (`Dip`/`Evl`/`policies`) → reconcile in the doctrine-land phase.
