...
- **8 kinds** (general/user IEL: `Icp`/`Ixn`/`Evl`/`Ath`/`Rev`/`Dth`/`Trm`/**`Wit`**); a **federation** IEL is the
  ...
  **`Wit`** is its governance kind (T2 — replaces `Evl` for the federation; cold-4 B1 / 2026-06-28; the federation
  ...
  | `Dth` | T2 | `t_authorize` | **sealed kill-anchor — deauthorize a grant**: via `anchors`, names the SEL `Trm`(s) it seals (**only** `Trm`s) — a lookup-SEL rescission `Trm` (delegation **or** doc-membership) — **plus a `kills[] = [{target, bound?}]`** declaration (the fail-secure rescission, alongside `anchors[]`; `target = hash('{topic}:{owner}:{data}')` — a flat domain-qualified hash, `topic` = `DLG_RSC_TOPIC` (delegate) / `DOC_RSC_TOPIC` (doc-member); `bound` = the grandfather cutoff, one concept in two custody modes — **the inline-public `kills[].bound` field for a delegate** (un-withholdable), **the gated `bound` role on the SEL `Trm` for a doc-member** (`kills[]` carries only the blind target — participant-blindness); **opaque to the IEL**; B1 fail-secure rework 2026-07-09, gated-role custody WF1 2026-07-13). The **polarity-inverse of `Ath`** (grant → deauthorize, same `t_authorize`). Carries **no roster delta**. **Forces a `Rot`.** Sealed-on-arrival; **sealed, non-terminal** (like `Rev`). |
