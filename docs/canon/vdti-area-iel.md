...
- **Write an operations note** (mirror kels' `operations/multi-party-governance.md`, in plain words): one
  ...
  **anchored SAD** (no issuance SEL) and revocation is a **`kills[]` declaration on the issuer's witnessed IEL `Rev`**
  ...
  a `t_use` content-content divergence freezes *that* SEL until an IEL burying seal resolves it; and under a
  **linear** owner IEL even those cannot fork — anchor-monotonicity makes a re-anchor inert (inv 4) — so the
  freeze window opens only on a real IEL fork, not a two-device SEL race — cold F1.)*
