...
- **There is no direct mode (first-seen, 2026-07-08).** Every identity is **federation-witnessed** — witnessing is
  ...
  federation-genesis bootstrap is **not** direct mode — the unwitnessed steps are the federation-infrastructure
  inceptions: genesis rooted in the config-pinned trust root (§1a), an added witness's `Fcp` pair rooted in the
  **witnessed** admitting `Wit` (cold-6 F4). The residual where a content fork can still form is a **witness
  compromise**, not an un-witnessed chain — §1e.)*
...
- **Dead-event gate (garbage reduction, 2026-07-01):** a witness that **already holds the burying seal-advancer** (any T2 seal-advancer — a `Rot`, `Evl`, `Ath`, `Rev`/`Dth`, or `Wit`)
  ...
  below the burying seal or through a below-seal fork. **A below-seal sealed event is declined too** — the witness **mirrors the seal-cap** (a parent below the tracked seal is inert, content _or_ sealed; revised 2026-07-11): it never reaches threshold, so it is neither dispute evidence nor able to retreat the clean seal. **The sealed leg is load-bearing — the backdate defense:** a below-seal sealed straggler must **not** be witnessed, or a total-key-compromise adversary could mint a fabricated historical fork years after the fact; a dispute is **≥ 2 accepted sealed branches** — a seal-vs-seal collision at one serial, or two branches that each seal at different serials above a double-signed fork (a provable witness double-sign, or — across disjoint federations — an author-side reserve double-reveal / member double-anchoring, cold-6 F3: nothing forces the two competing rebinds' anchors onto one device position, so the sequential shape double-reveals nothing, but the strict-majority quorum overlap still names a double-dealer — one member anchored both siblings, at one device position (double-reveal) or successive ones (double-anchoring), either confirmable from the token). **The gate generalizes — you can't seal a buried chain (dead-on-ascent, Jason 2026-07-11):** a lineage is dead from its **first-seen loss** at any position, not only below a burying seal. A selected witness that first-seen-accepted the winner at a fork **declines every descendant of the loser — content _or_ a `Rot`/`Evl` seal forged on it** — so a buried branch never gathers the majority a seal needs; a seal does not revive it. This is what makes the **honest** case single-winner (inv 13): only the branch that won first-seen stays live, so an honest fork yields **one** sealed branch → Active. A dispute needs **two accepted-lineage branches** (a double-sign at the fork, or disjoint federations); the verdict counts sealed **branches**, per branch, wherever their seals sit — not seal positions. **The content leg is an efficiency gate, not load-bearing:** deadness ascends
...
- **Receipts ENUMERATE the branches; the data decides (reframed 2026-06-23; re-keyed post-floor 2026-07-02):**
  ...
  branch-level walk** (inv 13 / 17: ≥ 2 branches each with a sealed event, per branch wherever their seals sit, over **retained**
